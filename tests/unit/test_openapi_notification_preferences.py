...
def test_complete_response_and_partial_patch_are_structurally_distinct(
    ...
    patch_schema = deepcopy(schemas["NotificationPreferencesPatch"])
    patch_schema["properties"]["quiet_schedule"] = schemas["QuietSchedulePatch"]
    OAS30Validator(patch_schema).validate({"alert_mode": "RING_ONLY"})
    ...
    for example in examples.values():
        OAS30Validator(patch_schema).validate(example["value"])
    for removed in (
        "incoming_calls_enabled",
        "notifications_enabled",
        "delivery_scope",
        "local_network_alert_mode",
        "remote_network_alert_mode",
    ):
        with pytest.raises(ValidationError):
            OAS30Validator(patch_schema).validate({removed: True})
...
def test_only_four_alert_modes_are_published(specification: dict[str, object]) -> None:
    schemas = specification["components"]["schemas"]  # type: ignore[index]
    expected = ["NONE", "RING_ONLY", "NOTIFICATION_ONLY", "RING_AND_NOTIFICATION"]
    assert schemas["NotificationPreferences"]["properties"]["alert_mode"]["enum"] == expected
    assert schemas["NotificationPreferencesPatch"]["properties"]["alert_mode"]["enum"] == expected
    assert set(schemas["NotificationPreferences"]["required"]) == {
        "version",
        "alert_mode",
        "quiet_schedule",
        "updated_at",
