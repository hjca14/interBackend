...
segundos). Criação transaciona Put condicionais; leituras de intenção/marcador e Query de respostas
são fortemente consistentes. Digests SHA-256 identificam escopo/chave sem persistir a
Idempotency-Key. Não há GSI, nova tabela ou replacement. O custo incremental é transação on-demand,
leituras fortes e Query das respostas existentes; ver o runbook da Fase 2D.
